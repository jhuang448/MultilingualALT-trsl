"""Modified from Transformer for ASR in the SpeechBrain style.
"""
...
class MyTransformerEncoder(TransformerEncoder):
    def __init__(
        ...
        self.lang_proj = nn.Linear(d_model, lang_dim)
...
class MyTransformerASR(MyTransformerInterface):
    ...
    def forward(self, src, tgt, wav_len=None, pad_idx=0, condition_emb=None):
        ...
        encoder_out_b, _, intermediate_outs = self.encoder(
            src=src,
            src_mask=src_mask,
            src_key_padding_mask=src_key_padding_mask,
        ...
        if self.decoder_condition:
            encoder_out = torch.concat((encoder_out_b, condition_emb), dim=2)
            encoder_out = self.custom_enc_module(encoder_out)
        else:
            encoder_out = encoder_out_b
        ...
        return encoder_out_b, encoder_out, decoder_out, intermediate_outs
