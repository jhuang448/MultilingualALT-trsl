...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            emb_switch = bool(torch.randint(0, 2, ()))
        else:
            emb_switch = True # for validation and test we always use FiLM

        if emb_switch == False:
            lang_cond = torch.ones_like(lang_cond) * 4 # unk

        e_lang = self.modules.emb(lang_cond)
        e_lang_exp = e_lang.repeat(1, src.shape[1], 1)

        enc_out_b, enc_out, pred, _ = self.modules.Transformer(
            src, tokens_bos, wav_lens, pad_idx=self.hparams.pad_index, condition_emb=e_lang_exp
        )

        # output layer for ctc log-probabilities
        logits = self.modules.ctc_lin(enc_out_b)
...
def dataio_prepare(hparams):
    ...
    def text_pipeline(wrd, lang):
        ...
        if lang == "en":
            ...
            assert(lang == "de")
            lang_cond = 3
        yield torch.LongTensor([lang_cond])
