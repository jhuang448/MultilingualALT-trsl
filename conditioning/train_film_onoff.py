...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        if hasattr(self.hparams, "film4"):  # this is to condition on the target language (it matters for transliteration targets)
            logits = self.modules.film4(logits, lang_cond, film_switch)
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.provides(
        "wrd", "tokens_list", "tokens_bos", "tokens_eos", "tokens", "lang_cond"
    )
    ...
    sb.dataio.dataset.set_output_keys(
        datasets, ["id", "sig", "wrd", "tokens_bos", "tokens_eos", "tokens", "lang_cond"],
    )
